...
async fn benchmark_data_loader() {
    const NUM_ITERATIONS: usize = 1000; // Adjust the number of iterations as needed
    ...
    let futures1 = (0..NUM_ITERATIONS).map(|_| loader.load_one(key1.clone()));
    let futures2 = (0..NUM_ITERATIONS).map(|_| loader.load_one(key2.clone()));
